-- Fecha: 2025-02-17
-- Propósito: Alinear la tabla production_data con los campos utilizados por el frontend
--            (metros_producidos / metros_vendidos) manteniendo compatibilidad con esquemas anteriores.
...
-- Sincronizar datos existentes si los alias quedaron vacíos
UPDATE production_data
SET
    year = COALESCE(year, period_year),
    month = COALESCE(month, period_month);

-- Migrar valores históricos desde unidades_* si los campos nuevos están vacíos
UPDATE production_data
SET
...
-- Nota: Mantener period_year/period_month para compatibilidad con reportes existentes.
--       Las APIs ahora consumen year/month/metros_, pero hacen fallback automático.
