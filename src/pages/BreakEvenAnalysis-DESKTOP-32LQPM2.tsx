  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="space-y-4">
        ...
        {(priceChange !== 0 || fixedCostChange !== 0 || variableCostRateChange !== 0 || enableMonteCarlo) &&
         currentResult.margenContribucion > 0 && (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div className="glass-card p-4 rounded-lg text-center">
                  ...
                  <div className="text-xs text-text-dim mt-1">
                    ...
                    {isFinite(getSimulatedValue('margenContribucionPorc')) && isFinite(currentResult.margenContribucionPorc)
                      ? ` ${(getSimulatedValue('margenContribucionPorc') - currentResult.margenContribucionPorc).toFixed(3)}`
                      : ''}
                  </div>
                </div>
              ...
              <div className="glass-card p-4 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {enableMonteCarlo && (
                    <div className="pt-2 border-t border-border/50 text-xs text-text-dim">
                      Monte Carlo activo • {monteCarloIterations} iteraciones
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="glass-card p-6 rounded-xl border border-warning/30 bg-warning/5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-text-muted text-sm">
                  No se puede simular escenarios con margen de contribución negativo. Los costos variables superan los ingresos actuales.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {cvuAnalysis.grado_apalancamiento_operativo > 3 ? (
                      <div className="text-danger text-xs mt-1">
                        ⚡ Muy alto - Una caída del 10% en ventas = {(cvuAnalysis.grado_apalancamiento_operativo * 10).toFixed(1)}% caída en utilidad
                      </div>
                    ) : cvuAnalysis.grado_apalancamiento_operativo > 2 ? (
                      <div className="text-warning text-xs mt-1">
                        ⚠️ Alto - Una caída del 10% en ventas = {(cvuAnalysis.grado_apalancamiento_operativo * 10).toFixed(1)}% caída en utilidad
                      </div>
                    ) : cvuAnalysis.grado_apalancamiento_operativo > 1.5 ? (
                      <div className="text-info text-xs mt-1">
                        📊 Moderado - Una caída del 10% en ventas = {(cvuAnalysis.grado_apalancamiento_operativo * 10).toFixed(1)}% caída en utilidad
                      </div>
                    ) : (
                      <div className="text-accent text-xs mt-1">
                        🔒 Bajo - Estructura financiera estable
                      </div>
                    )}
                  </div>
              ...
              <div className="glass-card p-4 rounded-lg">
                ...
                <div className="space-y-1 text-xs text-text-muted">
                  {targetProfit > 0 && (
                    <div>• Necesitas incrementar ventas {((cvuAnalysis.ventasParaUtilidad / currentResult.ingresos - 1) * 100).toFixed(1)}% para alcanzar la utilidad objetivo</div>
                  )}
                  {cvuAnalysis.margenSeguridadPorcentual < 10 && (
                    <div>• Considera reducir costos fijos o mejorar márgenes</div>
                  )}
                  <div>• Cada 1% de aumento en ventas impacta {cvuAnalysis.grado_apalancamiento_operativo.toFixed(1)}% en la utilidad</div>
                </div>
              </div>
            </div>
          </div>
